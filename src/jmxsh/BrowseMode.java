...
class BrowseMode extends Mode {
	...
	private String getAttropMenu() {
		...
		List<MBeanAttributeInfo> attributes = Arrays.asList(attrArray);
		Collections.sort(attributes, (MBeanAttributeInfo a1, MBeanAttributeInfo a2) -> a1.getName().toLowerCase().compareTo(a2.getName().toLowerCase()));

		List<MBeanOperationInfo> operations = Arrays.asList(operArray);
		Collections.sort(operations, (MBeanOperationInfo m1, MBeanOperationInfo m2) -> m1.getName().toLowerCase().compareTo(m2.getName().toLowerCase()));
	...
	String getMenu() {
		...
		Collections.sort(menu, (String s1, String s2) -> s1.toLowerCase().compareTo(s2.toLowerCase()));
